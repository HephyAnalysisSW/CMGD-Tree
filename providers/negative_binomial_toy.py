...
@dataclass
class NegativeBinomialToyStream(PlotConfigProvider):
    n_features: int
    n_classes: int
    batch_size: int
    n_batches: int
    ...
    def next_batch(self) -> StreamBatch:
        ...
        lam = self._rng.gamma(shape=self.shape, scale=mu / np.float32(self.shape)).astype(self.dtype)
        target_stats = self._rng.poisson(lam).astype(self.dtype)
