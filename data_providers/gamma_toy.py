...
@dataclass
class GammaToyStream(PlotConfigProvider):
    n_features: int
    n_classes: int
    batch_size: int
    n_batches: int
    ...
    def __post_init__(self) -> None:
        ...
        self._w = self._rng.normal(
            loc=0.0,
            scale=0.3 / np.sqrt(max(self.n_features, 1)),
            size=(self.n_features, self.n_classes),
        ).astype(self.dtype)
        self._bias = np.full((self.n_classes,), np.log(1.5), dtype=self.dtype)
        for c in range(self.n_classes):
            self._bias[c] += 0.1 * (c % 4)
    ...
    def next_batch(self) -> StreamBatch:
        ...
        theta = mu / np.float32(self.shape)
        target_stats = self._rng.gamma(shape=self.shape, scale=theta).astype(self.dtype)
